...
onMounted(async () => { const [settings, appInfo] = await Promise.all([window.filmLibrary.settings.get(), window.filmLibrary.app.info()]); if (settings.ok) Object.assign(form, settings.data); if (appInfo.ok) info.value = appInfo.data; });
async function save(): Promise<void> { saving.value = true; const result = await window.filmLibrary.settings.update(form); if (result.ok) { Object.assign(form, result.data); library.settings = result.data; ElMessage.success('设置已保存'); } else ElMessage.error(result.error.message); saving.value = false; }
...
<template>
  <div class="page-wrap settings-page"><div class="page-heading"><div><div class="eyebrow">LOCAL CONFIGURATION</div><h1 class="page-title">设置</h1><p class="page-caption">调整本地目录、扫描和悬浮预览行为。</p></div><el-button type="primary" :loading="saving" @click="save">保存设置</el-button></div>
    <div class="settings-grid"><section class="settings-card"><div class="settings-title"><Setting /><span>应用数据</span></div><div class="data-row"><span>数据库位置</span><code>{{ info?.databasePath || '读取中…' }}</code></div><div class="data-row"><span>日志目录</span><code>{{ info?.logsDirectory || '读取中…' }}</code></div><div class="settings-actions\"><el-button size="small" @click="openFolder('data')"><FolderOpened />打开数据目录</el-button><el-button size="small" @click="openFolder('logs')">打开日志目录</el-button></div></section>
      <section class="settings-card"><div class="settings-title"><VideoCamera /><span>卡片与预览</span></div><div class="setting-row"><label>默认卡片宽度</label><el-input-number v-model="form.cardSize" :min="160" :max="360" :step="10" /><span>px</span></div><div class="setting-row"><label>悬浮延迟</label><el-input-number v-model="form.hoverDelayMs" :min="100" :max="3000" :step="50" /><span>ms</span></div><div class="setting-row"><label>图片轮播间隔</label><el-input-number v-model="form.slideshowIntervalMs" :min="500" :max="10000" :step="100" /><span>ms</span></div><div class="setting-row"><label>每页数量</label><el-input-number v-model="form.pageSize" :min="12" :max="200" :step="12" /></div></section>
      ...
      <section class="settings-card wide"><div class="settings-title"><span>可选 ffprobe</span><span class="muted">不配置也不影响导入、NFO 和预览播放</span></div><el-input v-model="form.ffprobePath" placeholder="留空则尝试使用 PATH 中的 ffprobe"><template #append><el-button @click="testFfprobe">测试</el-button></template></el-input><p v-if="ffprobeResult" class="test-result">{{ ffprobeResult }}</p></section>
    </div>
  </div>
</template>

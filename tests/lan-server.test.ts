...
describe('localhost read-only web server', () => {
  ...
  it('serves a CSP-restricted browser client with the desktop-style navigation shell', async () => {
    ...
    expect(script).toContain('film-preview-triggers');
    expect(script).toContain('film-preview-trigger');
    ...
    expect(script).toContain('popup.dataset.side');
    ...
    expect(script).toContain('generation !== mediaGeneration');
    expect(script).toContain('createPreviewPagination');
    expect(script).toContain("'预览图片切换'");
    expect(script).toContain("'预览片段切换'");
    ...
    expect(styles).toContain('right: 9px; bottom: 9px');
    expect(styles).toContain('flex-direction: column');
    expect(styles).toContain('.film-preview-triggers');
    expect(styles).toContain('.film-preview-trigger:hover');
    expect(styles).not.toContain('.web-card-preview-channels');
    expect(styles).toContain('.web-card-preview-pagination');
    expect(styles).toContain('.web-preview-dot.active');
    expect(styles).toContain('.web-segment-preview-label');
    expect(styles).toContain('.comment-gallery-group:not(:only-child) { width: max-content');

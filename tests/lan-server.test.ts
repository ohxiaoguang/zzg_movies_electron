...
describe('localhost read-only web server', () => {
  ...
  it('serves a CSP-restricted browser client with the desktop-style navigation shell', async () => {
    ...
    expect(script).toContain('createPreviewPlayer');
    expect(script).toContain('createOriginalPlayback');
    expect(script).toContain("actionButton('预览视频'");
    expect(script).toContain("actionButton('图片图库'");
    expect(script).toContain("actionButton('原片'");
    ...
    expect(script).toContain("client.media('previews', filmId)");

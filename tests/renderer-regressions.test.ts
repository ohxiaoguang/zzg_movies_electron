...
describe('renderer regressions', () => {
  ...
  it('uses card-level preview triggers for annotated segments and image channels', () => {
    ...
    expect(popup).toContain("'popup-media-comment': activeChannel === 'comments'");
  ...
  it('uses a full-width detail workbench and exposes segment titles on timeline nodes', () => {
    ...
    expect(detailPlayer).toContain('window.filmLibrary.films.cancelPreview(partId)');
    expect(detailPlayer).toContain("mediaUrl(playbackSourceMode.value === 'direct' ? 'original-part' : 'part', selectedPartId.value)");
    expect(detailPlayer).toContain('@error="onPlaybackError"');
    expect(detailPlayer).toContain('crossorigin="anonymous"');
    expect(detailPlayer).toContain('原片无法直接播放，正在自动准备兼容版本');
    expect(detailPlayer).toContain('正在准备兼容版本…');
    expect(detailPlayer).toContain('window.filmLibrary.films.updatePartVr(partId, value)');
    expect(detailPlayer).toContain('是否 VR');
    expect(detailPlayer).toContain('是（360°）');
    expect(detailPlayer).toContain('SphericalVideoRenderer');
    expect(detailPlayer).toContain('class="vr-video-canvas"');
    expect(detailPlayer).toContain('拖动转动视角 · 滚轮缩放');
    ...
    expect(drawer).toContain('detailPlayer.value?.releasePlayback()');
    expect(drawer).toContain('v-if="modelValue"');

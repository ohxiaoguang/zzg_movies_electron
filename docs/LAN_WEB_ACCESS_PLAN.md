...
1. 局域网模式禁止匿名管理。
2. 桌面端首次进入时设置账号密码，网页端使用同一套账号密码登录。
3. 密码只保存加盐摘要；网页登录后使用运行期令牌，退出或删除凭据文件后失效。
4. 令牌不能以明文写入日志。
5. 限制登录请求频率。
6. 严格限制 CORS/Origin。
7. 写操作必须防止 CSRF。

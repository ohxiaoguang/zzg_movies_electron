...
- film_segment：绑定到具体影片文件的起止时间、标题、批注和精彩预览开关。
...
设置页可以连接一个专用的 GitHub 私有仓库，并把可跨电脑恢复的整理数据提交为 `library-backup.json`。备份只包含影片文件名、文件大小、可用的影片时长、收藏状态、自定义分类关系和精彩片段；不包含影片原文件、来源路径、盘符、海报、缓存、账号凭据或 SQLite 数据库。

- 影片先按包含扩展名的文件名匹配；同名候选再使用文件大小和允许两秒误差的影片时长辅助确认。仍然不能唯一确认的记录不会自动恢复。
- 恢复前会显示唯一匹配、未找到、同名冲突及可恢复片段数量。确认后只覆盖匹配影片的收藏、分类和片段，其他影片信息保持不变。
...
- “已整理”和“收藏”页面可把当前筛选结果导出为 UTF-8 CSV，固定字段为文件名、NFO 标题、我的分类、演员、NFO 摘要和精彩片段。精彩片段使用 JSON 数组保存所属文件名、相对路径、分段标识、标题、起止秒数和可读时间段；导出不受当前分页大小限制。
...
影片卡片始终保持 2:3 poster。没有 poster 或 thumb 时，应用会采用与 Jellyfin Screen Grabber 相同的策略：通过 FFmpeg 提取主视频 10% 位置的画面作为自动封面；无法读取时长时使用第 10 秒，并将结果缓存。源视频大小、修改时间或抓帧策略变化后缓存会自动失效。鼠标进入后按设置等待，默认 450ms，在 body 上显示不参与网格布局的 16:9 横向悬浮窗：

- 有精彩片段标注时，按标注顺序播放原片中的对应时间范围。
...
- 正常播放、点击片段跳转和连续精彩预览共用同一个播放器，不会创建第二套“片段播放器”。详情页左侧在“我的分类”下方折叠展示“基本信息 / 详细信息”，右侧播放器下方只保留“片段标注 / 图片”；标注时可用空格播放/暂停，`I` 设置起点，`O` 设置终点。桌面端和网页端的播放器按钮及方向键统一按设置的步进秒数快退/快进，播放器获得焦点后仍保持相同步进；按住 Shift 时使用独立设置的微调步进。
- 客户端播放器可按具体影片文件保存“是否 VR”设置；第一版支持单目 360°等距柱状视频，以 WebGL 球面视图播放，并提供鼠标/触摸拖动、滚轮缩放、视角复位、静音和全屏操作。普通视频默认保持平面播放器。
- 客户端详情播放器优先通过安全的 Range 路由读取原片，让 Chromium 尝试系统/显卡硬件解码；只有实际加载或解码失败时才自动切换到兼容转码缓存，并在播放器标题旁显示当前模式。
- 每条片段绑定到具体单文件或 CD/Disc 文件，客户端标注界面只填写起止时间、标题和“纳入精彩预览”开关；不会修改原视频或 NFO。数据库继续兼容旧版本已经保存的批注数据，但当前界面不再展示或编辑批注。
- 客户端图片页只显示缩略图；点击后使用接近全屏的大图查看器展示，并支持按钮或方向键左右切换。
- 客户端可以创建、修改和删除标注。局域网页只读展示标注，可点击播放单段或自动衔接播放所有精彩片段。
- 客户端悬浮预览只播放精彩片段；没有精彩片段时使用图片轮播。
- 网页范围播放不会写入正常的影片续播位置。需要转码时只处理目标时间范围，并缓存生成的 HLS 分片。
- 如果扫描发现影片文件大小或修改时间发生变化，旧片段会提示检查时间点。

## 自定义媒体协议和安全设计

- 客户端首次启动时设置账号和密码，之后每次打开都需要登录；可选择只记住账号，不保存明文密码。
- 局域网网页使用同一套账号密码登录。凭据保存在应用数据目录的 `account-credentials.json` 中，文件只包含用户名、随机盐和密码摘要；关闭应用并手动删除该文件即可重新设置。

主进程在 app.ready 前注册特权协议 film-media://，通过 protocol.handle() 提供：

~~~text
film-media://asset/<asset-uuid>
film-media://preview/<film-uuid>  （旧版兼容路由，界面不再使用）
...
- 预览不播放：检查精彩片段对应的原片是否仍可访问；损坏或不兼容且无法转码的视频会回退到图片。

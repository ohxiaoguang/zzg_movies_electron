...
向 GitHub 推送语义化版本 Tag（例如 `v1.2.3` 或 `1.2.3`）会触发 `.github/workflows/release.yml`。工作流在 Windows Runner 中执行类型检查、Lint、测试、Forge 打包和生产包冒烟验证，然后创建同名 GitHub Release，上传安装程序、ZIP、Squirrel `RELEASES` 和增量包。

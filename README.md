...
- film_file：逻辑影片下的单文件或 CD/Disc 分段文件、指纹和缺失状态。
- film：文件路径、快速指纹、NFO 元数据、评分、收藏、备注和 missing 状态。
...
- MKV 预览会优先使用 ffprobe 判断编码：H.264/AAC 仅实时重封装为浏览器可播放的 MP4 流，HEVC 或其他不兼容编码实时转为 H.264/AAC；即使文件扩展名是 MKV、内部实际为 MPEG-TS 也可处理。转码流不会写入来源目录，关闭预览时立即终止。
...
ffprobe 是可选的。未配置时仍可扫描、读取 NFO、显示海报并播放已有预览。配置后使用参数数组和 shell: false 读取时长、分辨率、编码器和容器。MKV 兼容预览会查找 ffprobe 同目录下的 ffmpeg，也会从系统 PATH 查找 ffmpeg/ffprobe；找不到 ffmpeg 时保留 Chromium 原生播放作为回退，并在日志中记录明确原因。
...
- SQLite v6 迁移、分页、扫描幂等、用户字段保护、移动识别、离线保护、归档。
